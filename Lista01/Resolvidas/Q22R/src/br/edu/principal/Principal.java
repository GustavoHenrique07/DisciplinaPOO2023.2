package br.edu.principal;

import java.util.Scanner;

public class Principal {

	public static void main(String[] args) {
		double vlr_sal, qtd_kw, vlr_kw, vlr_reais, desc, vlr_desc;
		
		Scanner sc = new Scanner(System.in);
		
		System.out.println("Digite o valor do salario minimo: ");
		vlr_sal = sc.nextDouble();
		
		System.out.println("Digite a quantidade de quilowatts consumida pela residência: ");
		qtd_kw = sc.nextDouble();
		
		vlr_kw = vlr_sal / 5;
		vlr_reais = vlr_kw * qtd_kw;
		desc = vlr_reais * 15 / 100;
		vlr_desc = vlr_reais - desc;
		
		System.out.println("O valor de cada quilowatt é: " + vlr_kw);
		System.out.println("O valor a ser pago de energia é: " + vlr_reais);
		System.out.println("O valor a ser pago de energia com desconto é: " + vlr_desc);
	}

}
